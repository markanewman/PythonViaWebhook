...
def process_message(state, message, tmp_dir_name, queue_service, blob_service):
	...
	if safe_execute(state, file_names['in'], file_names['out']):
		...
		blob_service.create_blob_from_path(__outgoing__, file_names['outname'], file_names['out'])
		queue_service.put_message(__outgoing__, file_names['outname'])
...
def stage_files(message, blob_service, blob_name, tmp_dir_name):
	...
	file_out_name = str(uuid.uuid4())
	file_out_path = Path(tmp_dir_name).joinpath(file_out_name)
	...
	return {'in' : file_in_path, 'out': file_out_path, 'outname': file_out_name}
